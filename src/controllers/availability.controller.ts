...
export const createRecurringAvailability = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const parsed = recurringAvailabilitySchema.parse(req.body);
    ...
    const conflict = existing.find(slot =>
      isOverlapping(parsed.startTime, parsed.endTime, slot.startTime, slot.endTime)
    );
    if (conflict) {
    ...
    const duplicate = existing.find(
      slot => slot.startTime === parsed.startTime && slot.endTime === parsed.endTime
    );
    if (duplicate) {
      return res.status(409).json({ success: false, message: 'This exact time slot already exists.' });
    }

    const slot = await prisma.recurringAvailability.create({
...
export const getRecurringAvailability = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const doctorId = (req as any).user.id;
...
export const updateRecurringAvailability = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (slot.doctorId !== doctorId) return res.status(403).json({ success: false, message: 'Access denied.' });
...
export const createCustomAvailability = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const doctorId = (req as any).user.id;
    const parsed = customAvailabilitySchema.parse(req.body);
    ...
    const existing = await prisma.customAvailability.findMany({
      where: { doctorId, date },
    });

    const conflict = existing.find(s =>
      isOverlapping(parsed.startTime, parsed.endTime, s.startTime, s.endTime)
    );
    if (conflict) {
      return res.status(409).json({
        success: false,
    ...
    const slot = await prisma.customAvailability.create({
...
export const getAvailabilityByDate = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const doctorId = (req as any).user.id;
    ...
    const customSlots = await prisma.customAvailability.findMany({
    ...
    const recurringSlots = await prisma.recurringAvailability.findMany({
