...
exports.findNextAvailableSlots = findNextAvailableSlots;
...
function isSlotInPast(date, slotStartTime) {
    const now = new Date();
    ...
    if (date.getUTCFullYear() === now.getUTCFullYear() &&
        ...
        if (slotHours < currentHours)
            return true;
        if (slotHours === currentHours && slotMins <= currentMins)
            return true;
    }
    return false;
}
function generateSlotsForDay(date, windows, appointments, doctorProfile, requestedStartTime, returnSingleSlot = false) {
    ...
    if (schedulingType === 'STREAM') {
        ...
        for (const window of windows) {
            const bookedInWave = appointments.filter(a => a.startTime === window.startTime && a.endTime === window.endTime).length;
 ...
 */
async function fetchDoctorScheduleData(doctorId, startDate, maxDaysAhead) {
    const maxDate = new Date(startDate);
    maxDate.setDate(maxDate.getDate() + maxDaysAhead);
    const [recurring, custom, appointments] = await Promise.all([
        prisma_1.default.recurringAvailability.findMany({ where: { doctorId } }),
        prisma_1.default.customAvailability.findMany({
 ...
 */
async function findNextAvailableSlots(doctorId, startDate, options = {}) {
    ...
    for (let calendarDay = 0; calendarDay < maxCalendarDaysLimit; calendarDay++) {
        ...
        const currentCustom = scheduleData.custom.filter(c => c.date.getTime() === currentDate.getTime());
        ...
        if (isWorkingDay) {
            workingDaysChecked++;
            let dayAppointments = scheduleData.appointments.filter(a => a.date.getTime() === currentDate.getTime());
            ...
            const generatedSlots = generateSlotsForDay(currentDate, windows, dayAppointments, doctor.doctorProfile, reqStartTime, options.returnSingleSlot);
